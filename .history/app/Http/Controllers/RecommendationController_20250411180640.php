...
class RecommendationController extends Controller
{
    public function getRecommendations(Request $request)
    {
        ...
        if (count($boughtProducts) < 4)
        {
            ...
            {
                $productVectors = array_merge($productVectors, Recommendation::where('product_id', $viewedProduct['product_id'])-get()->toArray());
            }

            $viewedProductsIds = collect($productVectors)->pluck('product_id')->toArray();

            for ($i = count($products) - 1; $i >= 0; $i--) {
                if (in_array($products[$i]['product_id'], $viewedProductsIds)) {
