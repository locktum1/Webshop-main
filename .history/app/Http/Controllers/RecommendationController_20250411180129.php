...
class RecommendationController extends Controller
{
    public function getRecommendations(Request $request)
    {
        ...
        if (count($boughtProducts) < 4)
        {
            ...
            {
                $productVectors = Recommendation::where('product_id', $viewedProduct['product_id'])->get();
            }


            $productIds = collect($productVectors)->pluck('product_id')->toArray();

            for ($i = count($productVectors) - 1; $i >= 0; $i--) {
                if (in_array($productVectors[$i]['product_id'], $productIds)) {
                    array_splice($productVectors, $i, 1);
                }
            };
