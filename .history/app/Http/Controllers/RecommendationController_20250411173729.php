...
class RecommendationController extends Controller
{
    public function getRecommendations(Request $request)
    {
        ...
        if (count($boughtProducts) < 5)
        {
            $viewedProducts = ProductViewed::where('user_id', Auth::id())->get();

            $products = [];

            foreach ($viewedProducts as $product)
            {
                $grabbed = Recommendation::where('product_id', $product->product_id)->get();
                foreach ($grabbed as $recommendation)
                {
                    $product = Recommendation::where('product_id', $product->product_id)->get();
                    $products[] = $product;
                }
            }
