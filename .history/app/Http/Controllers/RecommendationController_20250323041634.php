...
class RecommendationController extends Controller
{
    public function getRecommendations(Request $request)
    {
        ...
        $boughtProducts = Bought_Product::all()->pluck('product_id')->toArray();

        $filteredProducts = collect($products)->reject(
            function ($product) use ($boughtProducts)
            {
                return in_array($product['product_id'],$boughtProducts);
            }->toArray()
        );
